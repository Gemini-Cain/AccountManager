...
class AccountManager(object):
	...
	def __init__(self):
		...
		self.file = "D:\\Code\\AccountManager\\test.txt"
	...
	def add_account(self, ac):
		...
		handler = dbhandler.DBHandler(self.file)
		if handler.is_exist(condition):
			raise KeyError
		else:
			condition["name"] = ac.get_name();
			condition["website"] = ac.get_website();
			condition["tag"] = ac.get_tag();
			condition["login_name"] = ac.get_name();
			condition["password"] = ac.get_name();
			handler.insert(condition)
